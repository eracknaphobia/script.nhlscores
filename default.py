...
nhl_logo = os.path.join(ADDON_PATH,'resources','nhl_logo.png')
api_url = 'http://statsapi.web.nhl.com/api/v1/schedule?date=%s&expand=schedule.teams,schedule.linescore,schedule.scoringplays'
headshot_url = 'http://nhl.bamcontent.com/images/headshots/current/60x60/%s@2x.png'
    ...
    while ADDON.getSetting(id="score_updates") == 'true' and not monitor.abortRequested():
        ...
        for game in json_source['dates'][0]['games']:
            ...
            try:
                ...
                headshot = headshot_url % player_id
            except:
                pass
            ...
            for new_item in NEW_GAME_STATS:
                ...
                for old_item in OLD_GAME_STATS:
                    ...
                    if new_item[0] == old_item[0]:
                        ...
                        if ((new_item[3] != old_item[3] and int(new_item[3]) != 0) or (
                                new_item[4] != old_item[4] and int(new_item[4]) != 0)) or (
                                new_item[5].upper().find('FINAL') != -1 and old_item[5].upper().find(
                                'FINAL') == -1) or (new_item[6] != old_item[6]):
                            # Game variables
                            ateam = new_item[1]
                            hteam = new_item[2]
                            ascore = new_item[3]
                            ...
                            if ADDON.getSetting(id="score_updates") != 'false':
                                ...
                                if ADDON.getSetting(
                                    id="goal_desc") == 'true' and notify_mode == 'score' and headshot != '': img = headshot
